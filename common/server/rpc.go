...
func NewGrpcServer(etcd *config.EtcdConfig, grpc *config.GrpcConfig, cancelFunc ...func()) *GrpcServer {
	return &GrpcServer{
		etcd,
		grpc,
		cancelFunc,
...
func (g GrpcServer) RunGrpc(c GrpcConfig) {
	...
	log.Printf("%s关闭成功", g.grpcConfig.Name)
}
